...
import me.sadboyz.freelo.entities.Applications;
 ...
 */
@Stateless
@Path("me.sadboyz.freelo.entities.applications")
public class ApplicationsFacadeREST extends AbstractFacade<Applications> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Applications entity) {
        super.edit(entity);
    }
    ...
    public Applications find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_JSON})
    public List<Applications> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
